function [sol_A, sol_B] = solve_ODE_SAR_AGENCY(r,mu_B,mu_G,tau,sigma,a_bar,lambda1,omega1,gamma1,type,theta)
    ...
    %expr_a = matlabFunction(...
        %taylor( 1/(1 + gamma*r*sigma^2*x*(1-x) ), x, 'ExpansionPoint',.5,'Order',9));
    
    expr_a = matlabFunction( 1/( theta+theta^2*gamma*r*sigma^2*x*(1-x) ) );
    ...
    guessFun = @(z) guessAShockAgency(z,y0_guess,r,mu_G,mu_B,tau,omega,gamma,sigma,expr,expr_a, a_bar,type,theta);
    ...
    guessFun = @(z) guessBShockAgency(z,y0_guess,r,mu_B,mu_G,omega,lambda,gamma,sigma,expr,expr_a,a_bar,AShock_fun,type,theta);
    solinit = bvpinit(xmesh, guessFun);
    
    bvpoptions = bvpset(Stats="on",Nmax=150000,AbsTol=1e-4,RelTol=1e-4);
    sol_BShock = bvp5c(ode_fun,bc_fun,solinit,bvpoptions);
    disp('From agency before shock')
