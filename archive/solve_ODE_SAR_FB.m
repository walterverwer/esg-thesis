function [sol_A, sol_B] = solve_ODE_SAR_FB(r,mu_B,mu_G,tau,sigma,a_bar,lambda1,omega1,type,theta)
    ...
    guessFun = @(z) guessAShock(z,y0_guess,r,mu_G,mu_B,tau,omega,expr,a_bar,type,theta);
    ...
    ode_fun = @(z,y) odeBShock(z,y,r,mu_B,mu_G,omega,lambda,expr,a_bar,AShock_fun,type,theta);
    bc_fun = @(ya, yb) bc(ya, yb, p_BB, p_GB);
    
    % obtain init
    xmesh = linspace(0,1,5000);
    y0_guess = [p_BB;0]; % start is known, V' is unknown. If code gives error, vary the derivative.
    guessFun = @(z) guessBShock(z,y0_guess,r,mu_B,mu_G,omega,lambda,expr,a_bar,AShock_fun,type,theta);
    solinit = bvpinit(xmesh, guessFun);
